...
from satellite1 import AsyncSatellite1Client, LedColor
...
async def pulse(
    ...
    while loop.time() < end:
        ...
        await satellite.led.render_frame([scale(color, brightness)] * 24)
...
async def rainbow(
    ...
    while loop.time() < end:
        frame = [
            wheel(int(index * 256 // 24 + loop.time() * speed * 256))
