"""I2C driver for the PCM5122 line-out DAC and its GPIO pins."""
...
class PCM5122GPIOPin:
    """Configuration for one PCM5122 GPIO pin."""
...
class PCM5122:
    ...
    Methods mirror the C++ driver for setup, mute control, and volume control.
    """
    ...
    def setup(self) -> None:
        ...
        with self._i2c as bus:
            ...
            # Set Ignore Clock Halt Detection and clear Disable Autoset.
    ...
    def set_mute_off(self) -> bool:
        """Unmute the DAC and report whether the register write succeeded."""
        self._muted = False
        return self._write_mute()

    def set_mute_on(self) -> bool:
        """Mute the DAC and report whether the register write succeeded."""
        self._muted = True
        return self._write_mute()
    ...
    def volume(self) -> float:
        """Return the currently requested normalized volume."""
        return self._volume
    ...
    def _write_volume(self) -> bool:
        # Higher DVC codes are quieter, matching the C++ driver's mapping.
        code = int(
